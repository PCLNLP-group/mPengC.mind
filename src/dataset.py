...
def get_input_data_batch_slice_map(input_ids, eod_id, rank, dis, eod_reset):
    ...
    for bs_i, _ in enumerate(range(len(input_ids))):
        ...
        batch_attention_mask[bs_i] = np.tril(np.ones(shape=(seq_length, seq_length), dtype=np.int8))
...
def get_input_data_batch_slice_map2(input_ids, position_id, rank, dis, eod_reset):
    """
    Generate position_id and attention_mask according to input_ids considering eod reset
    ...
    seq_length = position_id.shape[1]
    tril_dev = np.tril(np.ones(shape=(seq_length, seq_length), dtype=np.int8))
    # Initialize position_ids and attention_mask
    batch_attention_mask = np.ones((len(position_id), seq_length, seq_length), dtype=np.int8)
    batch_position_ids = np.ones((dis, seq_length), dtype=np.int32)

    # Loop through batches
    for bs_i in range(len(position_id)):
        # Get normal position_ids and attention_mask
        batch_attention_mask[bs_i] = tril_dev
        # Find eod_of_document
        local_ids = position_id[bs_i]
        batch_position_ids[bs_i] = np.arange(seq_length)
        eod_index = batch_position_ids[bs_i, local_ids == 0].astype(np.int32)[1:] - 1
        for i in range(eod_index.size):
            # Reset position_ids and attention_mask considering EOD
            index = eod_index[i]
            batch_attention_mask[bs_i, (index + 1):, :(index + 1)] = 0
    return input_ids, position_id, batch_attention_mask


def get_input_data_batch_slice_map3(input_ids, position_id, eod_index, rank, dis, eod_reset):
    """
    Generate position_id and attention_mask according to input_ids considering eod reset
    ...
    position_id = position_id[rank * dis: (rank + 1) * dis]
    input_ids = input_ids[rank * dis: (rank + 1) * dis]
    eod_id = eod_index[rank * dis: (rank + 1) * dis]
    if not eod_reset:
        return position_id

    seq_length = position_id.shape[1]
    # Initialize position_ids and attention_mask
    batch_attention_mask = np.ones((len(position_id), seq_length, seq_length))
...
def create_dataset(batch_size, data_path, device_num=1, rank=0, drop=True, full_batch=False, data_start_index=0,
    ...
    dataset = ds.MindDataset(data[data_start_index:], columns_list=[column_name, "position_id"],
                             shuffle=False, num_samples=num_samples)
    ...
    map_func = (lambda input_ids, position_id: get_input_data_batch_slice_map2(input_ids, position_id, rank, dis, eod_reset))
    # If eod_reset enabled, another two inputs will be generated through input_ids
    num_parallel_workers = 8
    dataset = dataset.map(input_columns=column_name, operations=type_cast_op, num_parallel_workers=num_parallel_workers)
    if eod_reset:
        dataset = dataset.batch(batch_size, drop_remainder=drop)
        # dataset = dataset.map(operations=map_func, input_columns=[column_name],
        #                      output_columns=[column_name, "position_id", "attention_mask"], num_parallel_workers=num_parallel_workers)
        dataset = dataset.map(operations=map_func,
                               input_columns=["input_ids", "position_id"],
                               output_columns=["input_ids", "position_id", "attention_mask"], num_parallel_workers=num_parallel_workers)

        dataset = dataset.map(input_columns="position_id", operations=type_cast_op, num_parallel_workers=num_parallel_workers)
        # dataset = dataset.map(input_columns="attention_mask", operations=type_cast_op_float, num_parallel_workers=8)
    else:
