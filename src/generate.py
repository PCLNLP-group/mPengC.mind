...
def compute_CrossEntropyLoss(logits, labels, mask):

    from .loss import CrossEntropyLoss
    logits = Tensor(logits, mstype.float32)
    input_mask = Tensor(mask)
    labels = Tensor(labels)

    loss = CrossEntropyLoss(logits, labels, input_mask)
...
def get_scores_100b(model_predict, item, tokenizer, pad_length=4096):
    input_sentence = item
    ...
    logits, mask = model_predict.predict(Tensor(np.array(tokens, np.int32)))
    loss = compute_CrossEntropyLoss(logits.asnumpy(), labels, mask.asnumpy())
...
def generate(model, origin_inputs, config):
    ...
    while valid_length < target_length:
        ...
        # print(f"log_probs: {log_probs}\nlog_probs[:]: {log_probs[0].asnumpy().shape}\n{log_probs[1].asnumpy().shape}")
...
def generate_100b(model, origin_inputs, config):
    """
    Text generation
    ...
    while valid_length < target_length:
        ...
        target_index = np.random.choice(len(p), p=p)
        # Stop judgment
        if p_args[target_index] == end_token or valid_length == target_length - 1 or p_args[target_index] == 0:
            outputs = input_ids
            break
...
def generate_100b_task(model, origin_inputs, config, top_p=1.0, top_k_num = 3, max_generate_length=100, duRepeate=True):
    """
    Text generation
    ...
    # Return valid outputs out of padded outputs
    length = np.sum(outputs != 0)
    outputs = outputs[0][:length+1]
    # outputs = outputs[0]
    return outputs

def generate_increment(model, origin_inputs, config, duRepeate=True):
    """
