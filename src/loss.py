...
class _Softmax(Cell):
    ...
    def __init__(self):
        ...
        self.max = P.ArgMaxWithValue(axis=-1, keep_dims=True)
        self.sub = P.Sub()
        self.exp = P.Exp()
        self.div = P.RealDiv()
...
class CrossEntropyLoss(Cell):
    ...
    def __init__(self):
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL, ParallelMode.SEMI_AUTO_PARALLEL):
            ...
            self.add = P.Add().add_prim_attr("keep_alive", True)
            self.add_label = P.Add().add_prim_attr("keep_alive", True)
...
class CrossEntropyLoss_eval(Cell):
    """
    Calculate the cross entropy loss.
    ...
    """
    def __init__(self):
        super(CrossEntropyLoss_eval, self).__init__()
