...
class PipeMultiHeadAttention(Cell):
    ...
    def __init__(self, batch_size,
        ...
        self.transpose_q = P.Transpose().shard(
            ((parallel_config.data_parallel, 1, parallel_config.model_parallel, 1),))
        self.transpose_k = P.Transpose().shard(
            ((parallel_config.data_parallel, 1, parallel_config.model_parallel, 1),))
        self.transpose_v = P.Transpose().shard(
            ((parallel_config.data_parallel, 1, parallel_config.model_parallel, 1),))
        ...
        if self.use_flash_attention:
            ...
            self.pad_tensor = Tensor(np.zeros((self.batch_size, self.n_head, self.src_seq_length, 16 - self.head_dim % 16)), dtype=compute_dtype)
    ...
    def _softmax(self, attention_scores):
        ...
        if self._is_ascend and self.softmax_dtype == mstype.float16 or not self._is_ascend:
            ...
            attention_probs = self.softmax_3d(
                self.softmax_reshape(attention_scores,
                          (shape[0], -1, shape[-1])))
    ...
    def _attn(self, query, key, value, attention_mask):
        ...
        query = self.real_div_q(query, factor)
        key = self.real_div_k(key, factor)
        score = self.batch_matmul1(query, key)
        ...
        if attention_mask is not None:
            ...
            attention_scores = self.add_mask(adder, attention_scores)
...
class PipeTransformerEncoder(Cell):
    ...
    def __init__(self,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation():
            ...
            if num_layers == 32:
                print(">>> 7B last Layer softmax-fp32")
                last_layer_softmax32 = True  # 7B lastLayer softmax-fp32

            for i in range(num_layers):
                if i == 31 and last_layer_softmax32:
                    softmax_compute_type = mstype.float32
