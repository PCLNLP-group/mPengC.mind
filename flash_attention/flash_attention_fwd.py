...
class FlashAttentionFwd(FlashAttention):
    ...
    def update_o_m_l_fp32(self,
        ...
        with self.tik_instance.else_scope():
            ...
            n1_tail = n1 - n1_front
            with self.tik_instance.new_stmt_scope(disable_sync=False):
                Oi_ub = self.tik_instance.Tensor(FP32, (n1_front, m, n0), name="Oi_ub", scope=UB)
                self.tik_instance.data_move(dst=Oi_ub, src=self.O_gm_workspace[o_gm_offset],
                                            sid=0, nburst=n1_front, burst=m * self.N0 // 8,
                                            src_stride=(self.Nq - m) * self.N0 // 8, dst_stride=0)

                with self.tik_instance.for_range(0, n1_front) as idx:
                    self.tik_instance.h_mul(Oi_ub[idx, :, :],
                                            Oi_ub[idx, :, :],
                                            li_new_rec_vec_ub)
                self.tik_instance.h_add(Oi_ub, Oi_ub, Pij_Vj_matmul_res_ub[:n1_front, :, :])
                self.tik_instance.data_move(dst=self.O_gm_workspace[o_gm_offset], src=Oi_ub, sid=0,
                                            nburst=n1_front, burst=block_h * self.N0 // 8,
                                            src_stride=0, dst_stride=(self.Nq - block_h_aligned) * self.N0 // 8)

            with self.tik_instance.new_stmt_scope(disable_sync=False):
                Oi_ub = self.tik_instance.Tensor(FP32, (n1_tail, m, n0), name="Oi_ub", scope=UB)
                o_cur_offset = o_gm_offset + n1_front * self.Nq * n0
                self.tik_instance.data_move(dst=Oi_ub, src=self.O_gm_workspace[o_cur_offset],
                                            sid=0, nburst=n1_tail, burst=m * self.N0 // 8,
                                            src_stride=(self.Nq - m) * self.N0 // 8, dst_stride=0)

                with self.tik_instance.for_range(0, n1_tail) as idx:
                    self.tik_instance.h_mul(Oi_ub[idx, :, :],
                                            Oi_ub[idx, :, :],
                                            li_new_rec_vec_ub)
                self.tik_instance.h_add(Oi_ub, Oi_ub, Pij_Vj_matmul_res_ub[n1_front:, :, :])
                self.tik_instance.data_move(dst=self.O_gm_workspace[o_cur_offset], src=Oi_ub, sid=0,
                                            nburst=n1_tail, burst=block_h * self.N0 // 8,
                                            src_stride=0, dst_stride=(self.Nq - block_h_aligned) * self.N0 // 8)
